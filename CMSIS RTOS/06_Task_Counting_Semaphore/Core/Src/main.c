...
/* USER CODE BEGIN PV */
volatile uint32_t producer_count = 0;
volatile uint32_t consumer_count = 0;
volatile uint32_t consumer_waiting = 0;

volatile uint32_t release_success = 0;
volatile uint32_t release_failed = 0;

volatile uint32_t semaphore_count = 0;
/* USER CODE END PV */
...
int main(void)
{
  ...
  DataSemaphoreHandle = osSemaphoreNew(10, 0, &DataSemaphore_attributes);
...
void StartProducerTask(void *argument)
{
  ...
  for(;;)
  {
	  ...
	  if(osSemaphoreRelease(DataSemaphoreHandle) == osOK)
		  release_success++;
	  else
		  release_failed++;

	  semaphore_count = osSemaphoreGetCount(DataSemaphoreHandle);
...
void StartConsumerTask(void *argument)
{
  ...
  for(;;)
  {
	  ...
	  osSemaphoreAcquire(DataSemaphoreHandle, osWaitForever);
