...
/* USER CODE BEGIN PV */
volatile uint32_t eventA_count = 0;
volatile uint32_t eventB_count = 0;

volatile uint32_t task3_wakeup_count = 0;
volatile uint32_t received_flags = 0;
volatile uint32_t task3_waiting = 0;
/* USER CODE END PV */
...
void StartTask03(void *argument)
{
  ...
  for(;;)
  {
      ...
      task3_waiting = 1;
      flags = osEventFlagsWait(myEvent01Handle,0x03,osFlagsWaitAll,osWaitForever);
      task3_waiting = 0;
