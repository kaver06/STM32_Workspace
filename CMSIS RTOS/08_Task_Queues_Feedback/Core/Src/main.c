...
volatile uint32_t put_success = 0;
volatile uint32_t put_failed = 0;
volatile uint32_t queue_count=0;

volatile uint32_t sent_data=0;
volatile uint32_t recieved_data=0;

volatile uint32_t producer_waiting = 0;
/* USER CODE END PV */
...
void StartProducerTask(void *argument)
{
  ...
  for(;;)
  {
	  ...
	  producer_count++;
	  producer_waiting = 1;
	  status=osMessageQueuePut(DataQueueHandle, &data, 0, 200);
	  producer_waiting = 0;
	  if(status==osOK)
	  {
		  sent_data=data;
	  ...
	  queue_count=osMessageQueueGetCount(DataQueueHandle);
	  osDelay(100);
  }
  /* USER CODE END 5 */
}
...
void StartConsumerTask(void *argument)
{
  ...
  for(;;)
  {
	  osMessageQueueGet(DataQueueHandle, &data, NULL, osWaitForever);
	  recieved_data=data;
	  consumer_count++;
	  queue_count=osMessageQueueGetCount(DataQueueHandle);
	  osDelay(1000);
